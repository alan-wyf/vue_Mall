...
export default {
  ...
  async created () {
    ...
    const res = await axios.get(this.$root.api.GoodsCategory, { params: { id: this.id } })
    console.log(res.data)
    this.clist = res.data.data.brotherCategory
    // eslint-disable-next-line no-unused-vars
    const id = this.clist[0].id
  ...
  methods: {
    async getlist (cid, page) {
      const res = await axios.get(`${this.$root.api.GoodsList}?categoryId=${cid}&page=${page}&size=20`)
      console.log(res)
