...
export default {
  ...
  methods: {
    ...
    async onAddCartClicked (skuData) {
      ...
      const cartRes = await axios.post(this.$root.api.CartAdd, { goodsId: resultPro[0].goods_id, number: skuData.selectedNum, productId: resultPro[0].id })
  ...
  async created () {
    ...
    const res = await axios.get(this.$root.api.GoodsDetail, { params: { id: this.id } })
